...
test("opens PlayStudy directly at the site root", async () => {
  ...
  assert.match(html, /href="\/playstudy\/styles\.css\?v=36"/);
  assert.match(html, /href="\/manifest\.webmanifest"/);
  assert.match(html, /src="\/pwa\.js\?v=36"/);
  assert.match(html, /src="\/playstudy\/player-gestures\.js\?v=36"/);
  assert.match(html, /src="\/playstudy\/app\.js\?v=36"/);
...
test("ships an early root-scoped landscape PWA bootstrap", async () => {
  ...
  assert.match(rootWorker, /player-gestures\.js\?v=36/);
  ...
  assert.match(pageSource, /<script defer src="\/pwa\.js\?v=36"/);
  assert.match(pageSource, /<script defer src="\/playstudy\/player-gestures\.js\?v=36"/);
  assert.match(pageSource, /<script defer src="\/playstudy\/app\.js\?v=36"/);
  assert.match(pageSource, /href="\/playstudy\/styles\.css\?v=36"/);
...
test("builds a complete root GitHub Pages PWA", async () => {
  ...
  assert.match(projectLegacyHtml, /src="\/playstudy-video-analysis\/playstudy\/app\.js\?v=36"/);
});
...
test("ships a minimal native-like library and full-screen mobile player", async () => {
  ...
  assert.match(focus, /data-focus-frame="-1"[^>]*>−微調整/);
  assert.match(focus, /data-focus-frame="1"[^>]*>＋微調整/);
